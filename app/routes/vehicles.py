...
from app.routes.dashboard import REPORT_LABELS
...
@login_required
def print_report():
    """Print-friendly version of the all_expiring report."""
    ...
    if report_type != "all_expiring":
        flash("Only All Expiring report can be printed.", "error")
        return redirect(url_for("main.reports", type=report_type, days=days))

    from app.routes.dashboard import Vehicle
        ...
        for doc_type in Vehicle.DOCUMENT_TYPES:
            ...
            for exp in expiries:
                if exp.expiry_date and exp.expiry_date <= window_end:
                    ...
                    days_left = (exp.expiry_date - today).days
                    expiring.append({
                        "field": doc_type,
    ...
    return render_template("print_report.html", 
                          results=results, summary=summary, days=days,
                          report_label=REPORT_LABELS.get("all_expiring", ""),
